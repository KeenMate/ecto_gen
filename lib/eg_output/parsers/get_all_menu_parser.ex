...
defmodule MyApp.Database.Parsers.GetAllMenuParser do
  @moduledoc """
  ...
  @spec parse_get_all_menu_result({:ok, Postgrex.Result.t()} | {:error, any()}) ::
          {:ok,
           [
             MyApp.Database.Models.GetAllMenuItem.t()
           ]}
          | {:error, any()}
  def parse_get_all_menu_result({:error, reason} = err) do
    Logger.error("Error occured while calling stored procedure",
      procedure: "get_all_menu",
  ...
  def parse_get_all_menu_result({:ok, %Postgrex.Result{rows: rows}}) do
    Logger.debug("Parsing successful response from database")

    parsed_results =
      rows
      |> Enum.map(&parse_get_all_menu_result_row/1)
  ...
  def parse_get_all_menu_result_row([
        menu_id,
        code,
        title_cs,
        title_en,
    ...
    {
      :ok,
      %MyApp.Database.Models.GetAllMenuItem{
        menu_id: menu_id,
        code: code,
        title_cs: title_cs,
        title_en: title_en,
  ...
  def parse_get_all_menu_result_row(_unknown_row) do
